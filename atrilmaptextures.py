#Alien Trilogy map obj/mtl generator script by Bobblen
map_file_name="L111LEV.MAP"
...
#now extract the faces from the map file
with open(map_file_name, 'rb') as inLEV:
    inLEV.seek(20)
    vertcountraw = inLEV.read(2)
    vertcount = int.from_bytes(vertcountraw, byteorder='little')
    
    vertstart=56
    vertend = (vertstart-1) + (vertcount*8) - 2
    
    quadcountraw = inLEV.read(2)
    quadcount = int.from_bytes(quadcountraw, byteorder='little')
    
    quadstart = vertend + 3
    quadend = quadstart + (quadcount*20) - 2
    
    z=0 #initialise counter
    
    inLEV.seek(quadstart)
    while (z<quadcount-1):
        xquadraw = inLEV.read(4)
        yquadraw = inLEV.read(4)
        zquadraw = inLEV.read(4)
        wquadraw = inLEV.read(4)
        texquadraw = inLEV.read(2) #the index of the texture in the BXfile, we can use this to look up the material and UV using our dictionaries
        texflagraw = inLEV.read(1)
        unknown1 = inLEV.read(1)
...
print("\nAlien Trilogy DOS Map Header Reader\n" + "\nmap file name is " + map_file_name
+ "\ntexure_mesh_file_names are " + map_texture_mesh_file_name_00 + "," + map_texture_mesh_file_name_01 + "," + map_texture_mesh_file_name_02 + "," + map_texture_mesh_file_name_03 + "," + map_texture_mesh_file_name_04
+ "\nvertex count (decimal) " + str(vertcount) + "\nface count (decimal) " + str(quadcount) + "\nvertex start offset (decimal) " 
+ str(vertstart) + "\nvertex end offset (decimal) " + str(vertend) + "\nfaces end offset (decimal) " + str(quadend))

print("\nbin2obj.exe " + map_file_name + " -soff " + str(vertstart) + " -eoff " + str(vertend) + " -stri 2 -vtyp 1 -fsof " + str(quadstart) + " -feof " + str(quadend) + " -fstr 4 -ftyp 1 -fquad")
